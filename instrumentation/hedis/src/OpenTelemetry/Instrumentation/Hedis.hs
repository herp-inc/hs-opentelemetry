...
import qualified OpenTelemetry.Trace.Monad as Otel (MonadTracer, TracerT (TracerT), getTracer)
...
-- 'Orig.RedisTx' cannot be an instance of 'Orig.MonadRedis'
...
doCheckedConnect tracer info = inSpan tracer "doConnect" info $ liftIO $ Connection info <$> Orig.checkedConnect info


-- | A function wrapping 'Orig.disconnect' with 'Otel.Tracer' using 'Otel.getGlobalTracerProvider'.
disconnect :: (Otel.MonadTracer m, MonadUnliftIO m, HasCallStack) => Connection -> m ()
disconnect conn = do
  tp <- Otel.getGlobalTracerProvider
  disconnect' tp conn
...
doDisconnect tracer Connection {originalConnection, connectInfo} = inSpan tracer "doConnect" connectInfo $ liftIO $ Orig.disconnect originalConnection


-- | A function wrapping 'Orig.withConnect' with 'Otel.Tracer' using 'Otel.getGlobalTracerProvider'.
withConnect :: (Otel.MonadTracer m, E.MonadMask m, MonadUnliftIO m, HasCallStack) => Orig.ConnectInfo -> (Orig.Connection -> m c) -> m c
withConnect info action = do
  tracer <- Otel.getTracer
  inSpan tracer "withConnect" info $ E.bracket (connect info) disconnect $ \(Connection {originalConnection}) -> action originalConnection
...
withCheckedConnect :: (Otel.MonadTracer m, E.MonadMask m, MonadUnliftIO m, HasCallStack) => Orig.ConnectInfo -> (Orig.Connection -> m c) -> m c
